...
function indexWord(index, word, entry) {
    ...
    if (index[word].some(isEntryEqualTo(entry))) {
        return;
    }
    index[word].push(entry);
}

function indexText(index, text, entry) {
    text.replace(/[\/.,"?=]/g, " ").split(/\s+/).forEach(function(word) {
        indexWord(index, word, entry);
    });
}
...
this.addEventListener("message", function(evt) {
    ...
    var catalog = evt.data;
    catalog.sections.forEach(function(section) {
        indexText(index, section.name, ["section", section.id]);
        indexText(index, section.description, ["section", section.id]);

        section.items.forEach(function(item) {
            indexText(index, item.name, ["item", item.id]);
            indexText(index, item.description, ["item", item.id]);

            item.tags.forEach(function(word) {
                indexWord(index, word, ["item", item.id]);
            });
        });
    });
